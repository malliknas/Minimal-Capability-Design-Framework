...
private static buildAppointmentFewShotPrompt(userInput: string, tier: string) {
    const tierPrompts = {
        Q1: `Examples:
...
Apply this pattern to: ${userInput}`,
        ...
        Q8: `Example 1: Book cardiology Tuesday 3pm
Output: Confirmed Cardiology Tuesday 3pm
Apply same pattern to: ${userInput}`
    };
...
Process: ${userInput}`,
    ...
    'Q4': `Navigation Examples:
Grid A1 to B3 avoid C2 → Head North 2 steps, East 1 step to B3, avoid C2
Lobby to Room 205 → Take elevator West, then North to Room 205
Apply to: ${userInput}`,
...
EXECUTION: Analyze using the specific systems and problems mentioned.`
  ...
  private static buildDiagnosticFewShotPrompt(userInput: string, tier: string): { fullPrompt: string; systemPrompt?: string } {
    const tierPrompts = {
      'Q1': `Examples:
Server won't start → Check: 1) Port 2) Service 3) Logs
Database timeout → Check: 1) Network 2) Auth 3) Service  
Process: ${userInput}`,

      'Q4': `Diagnostic Examples:
Server won't start port 8080 → Check: 1) Port 8080 status 2) Service status 3) Startup logs
Apply pattern to: ${userInput}`,
