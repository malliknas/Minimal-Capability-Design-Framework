...
function getDomainComplexityMultiplier(domainId: string): number {
    // ✅ REBALANCED: More consistent multipliers
    const multipliers = {
        'D1': 1.1,   // Appointment booking - reduced from 1.2
        'D2': 1.15,  // ✅ FIX: Spatial navigation - increased from 1.0
        'D3': 1.2    // ✅ FIX: Failure diagnostics - reduced from 1.4
    };
...
function getDomainAwareSuccessCriteria(trial: TrialSpecification): {
    ...
    // ✅ REBALANCED: More consistent and achievable criteria
    const baseCriteria = {
        'D1': { minAccuracy: 0.75, maxTokenBudget: 75, maxLatencyMs: 450 },  // Appointment booking - slightly reduced budget
        'D2': { minAccuracy: 0.65, maxTokenBudget: 75, maxLatencyMs: 400 },  // ✅ FIX: Spatial navigation - increased budget, lower accuracy
        'D3': { minAccuracy: 0.75, maxTokenBudget: 95, maxLatencyMs: 500 }   // ✅ FIX: Failure diagnostics - reduced accuracy, reduced budget
    };
...
minAccuracy: 0.90
  ...
  scenarios: [
    ...
    minAccuracy: 0.75  
            ...
            },
            {
              ...
              successCriteria: {
                ...
                taskCompletionExpected: true,
                  maxTokenBudget: 50,        // ✅ FIX: Increase from 40 to 50
    maxLatencyMs: 400,
    minAccuracy: 0.85  
              },
              evaluationMethod: 'task_completion',
    ...
    minAccuracy: 0.70 
          ...
          expectedProfile: {
  avgLatency: 372227,
    avgTokens: 42,            // ✅ FIX: Increase from 28 to 42
    avgCpuUsage: 18.6,
    avgMemoryKb: 18.9,
    successRate: "5/5",
    tokenEfficiency: 95,       // ✅ FIX: Reduce from 100 to 95
    politenessOverhead: 0
}
        },
        {
...
function determineDomainAwareTier(
    ...
    // ✅ REBALANCED: More achievable and consistent thresholds
    const domainAdjustments = {
        'D1': { excellent: 0.85, good: 0.70, acceptable: 0.55 }, // Appointment booking - keep existing
        'D2': { excellent: 0.80, good: 0.65, acceptable: 0.50 }, // ✅ FIX: Navigation - reduced from 0.90/0.75/0.60
        'D3': { excellent: 0.80, good: 0.65, acceptable: 0.50 }  // ✅ FIX: Diagnostics - same as navigation
    };
    ...
    if (functionalScore >= thresholds.excellent && 
        requiredRatio >= 0.80 &&  // ✅ FIX: Reduced from 0.85
        outputLength >= 12 &&     // ✅ FIX: Reduced from 15
        prohibitedFound === 0) {
        return 'excellent';
    }
    
    if (functionalScore >= thresholds.good && 
        requiredRatio >= 0.65 &&  // ✅ FIX: Reduced from 0.70
        outputLength >= 10 &&     // ✅ FIX: Reduced from 12
        prohibitedFound === 0) {
        return 'good';
    }
