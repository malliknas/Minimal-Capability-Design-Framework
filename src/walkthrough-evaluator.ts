...
function buildAppointmentMCDPrompt(userInput: string): { fullPrompt: string; systemPrompt?: string } {
    ...
    let response = '';
    if (slots.type && slots.date && slots.time) {
        // Complete booking
        response = `Confirmed: ${slots.type}, ${slots.date} ${slots.time}`;
    } else {
        // Missing information
        const missing = [];
        if (!slots.type) missing.push('appointment type');
        if (!slots.date) missing.push('date');  
        if (!slots.time) missing.push('time');
        
        response = `Missing: ${missing.join(', ')} for ${slots.type || 'appointment'}`;
    }
    
    return {
        fullPrompt: `Process appointment booking request: "${userInput}"

Required information: [type], [date], [time]
Found: ${JSON.stringify(slots)}
Response: ${response}`,
        systemPrompt: "You are a medical appointment scheduler. Extract appointment details and provide structured responses."
    };
}

// ✅ SPATIAL NAVIGATION: Directional guidance
function buildNavigationMCDPrompt(userInput: string): { fullPrompt: string; systemPrompt?: string } {
    const navElements = extractNavigationElements(userInput);
    
    let response = '';
    if (navElements.destination) {
        if (navElements.obstacles) {
            response = `Navigate to ${navElements.destination}, avoid ${navElements.obstacles}`;
        } else {
            response = `Navigate ${navElements.direction || 'directly'} to ${navElements.destination}`;
        }
    } else {
        response = `Specify destination for navigation guidance`;
    }
    
    return {
        fullPrompt: `Process navigation request: "${userInput}"

Elements: destination, direction, obstacles
Found: ${JSON.stringify(navElements)}
Response: ${response}`,
        systemPrompt: "You are a precision navigation system. Provide clear directional guidance."
    };
}

// ✅ FAILURE DIAGNOSTICS: Systematic troubleshooting
function buildDiagnosticsMCDPrompt(userInput: string): { fullPrompt: string; systemPrompt?: string } {
    ...
    let response = '';
    if (symptoms.complexity === 'high') {
        response = `Complex issue detected. Escalate to expert: ${symptoms.summary}`;
    } else if (symptoms.systemType) {
        const checks = getDiagnosticChecks(symptoms.systemType);
        response = `Diagnostic: ${symptoms.systemType} → Check: ${checks.join(', ')}`;
    } else {
        response = `Identify system type for diagnostic protocol`;
    }
    
    return {
        fullPrompt: `Process diagnostic request: "${userInput}"

Symptoms analysis: ${JSON.stringify(symptoms)}
Diagnostic approach: ${response}`,
        systemPrompt: "You are a senior systems engineer. Provide systematic diagnostic procedures."
    };
}
...
function countTokensAccurately(text: string): number {
    ...
    const baseTokens = words;
    const punctuationTokens = Math.ceil(punctuation * 0.3);
    const numberTokens = Math.ceil(numbers * 0.4);
    const specialTokens = Math.ceil(specialChars * 0.2);
    
    return baseTokens + punctuationTokens + numberTokens + specialTokens;
}
...
function validatePercentage(value: number, label: string = 'metric'): number {
    ...
    if (clamped !== value && Math.abs(value) > 0.01) {
...
function calculateEfficiencySafe(
    ...
    try {
        // Domain-specific efficiency baselines
...
// ✅ NEW: Spatial navigation validation
function isValidNavigationOutput(output: string, userInput: string): boolean {
    const cleanOutput = output.toLowerCase();
    
    // Check for directional guidance
    const hasDirection = /\b(north|south|east|west|left|right|straight|forward|back)\b/i.test(output);
    
    // Check for distance/measurement
    const hasDistance = /\b(\d+\s*(m|meter|step|foot|feet)|\d+m)\b/i.test(output);
    
    // Check for destination reference
    const hasDestination = /\b(to|toward|destination|location|building|room|exit|entrance)\b/i.test(output);
    
    // Check for obstacle awareness
    const hasObstacleAwareness = /\b(avoid|around|blocked|construction|wet floor|obstacle)\b/i.test(output);
    
    // Check for actionable navigation content
    const hasActionableNavigation = 
        cleanOutput.includes('navigate') ||
        cleanOutput.includes('go') ||
        cleanOutput.includes('head') ||
        cleanOutput.includes('proceed') ||
        cleanOutput.includes('move');
    
    // Avoid template responses
    const isNotTemplate = 
        !cleanOutput.includes('[start]') &&
        !cleanOutput.includes('[end]') &&
        !cleanOutput.includes('[destination]') &&
        !cleanOutput.includes('[direction]');
    
    // Must provide either specific navigation OR clear guidance request
    const hasSpecificNavigation = hasDirection && (hasDistance || hasDestination);
    const hasGuidance = hasActionableNavigation && hasDestination;
    
    return isNotTemplate && (hasSpecificNavigation || hasGuidance || hasObstacleAwareness);
}
