...
export class BrowserModelLoader {
   ...
   private static engineProtection = new Set<string>();
    private static protectionTimeouts = new Map<string, NodeJS.Timeout>();
      private static enhancedCreateEngine: typeof webllm.CreateMLCEngine | null = null;
private static webgpuAdapter: GPUAdapter | null = null;
private static webgpuInitialized: boolean = false;
...
static async initializeWebGPUAdapter(): Promise<GPUAdapter | null> {
    if (this.webgpuInitialized && this.webgpuAdapter) {
        return this.webgpuAdapter;
    }
    ...
    try {
        ...
        let adapterOptions: GPURequestAdapterOptions = {};
        
        if (isWindows) {
            // On Windows, omit powerPreference to avoid warnings
            BrowserLogger.log('🖥️ Windows detected: Using default GPU adapter configuration', 'ModelManager');
        } else {
            // On other platforms, request high-performance GPU
            adapterOptions.powerPreference = 'high-performance';
            BrowserLogger.log('🚀 Non-Windows platform: Requesting high-performance GPU', 'ModelManager');
        }

        // Request adapter with platform-specific options
        const adapter = await navigator.gpu.requestAdapter(adapterOptions);
        
        if (!adapter) {
            BrowserLogger.log('⚠️ No GPU adapter available - WebGPU not supported', 'ModelManager');
        ...
        BrowserLogger.log(`✅ GPU Adapter initialized: ${adapterInfo.vendor || 'Unknown'} ${adapterInfo.device || 'GPU'}`, 'ModelManager');
        ...
        BrowserLogger.log(`❌ WebGPU adapter initialization failed: ${error?.message}`, 'ModelManager');
        this.webgpuInitialized = true;
        return null;
    }
}
...
private static getCacheTTL(context?: ModelLoadingContext): number {
	...
	static async checkWebGPUHealth(): Promise<{
    available: boolean;
    adapter: boolean;
    ...
    try {
        if (!navigator.gpu) {
            return {
                ...
                adapter: false,
                performance: 'unknown',
                warnings: ['WebGPU not supported in this browser']
            };
        }
        
        const adapter = await BrowserModelLoader.initializeWebGPUAdapter();
        if (!adapter) {
            warnings.push('No GPU adapter available');
            return {
                available: true,
                adapter: false,
                performance: 'low',
        ...
        // Estimate performance based on adapter info
        const adapterInfo = adapter.info;
        let performance: 'high' | 'medium' | 'low' = 'medium';
        
        if (adapterInfo.vendor?.toLowerCase().includes('nvidia') || 
            adapterInfo.vendor?.toLowerCase().includes('amd')) {
            performance = 'high';
        } else if (adapterInfo.vendor?.toLowerCase().includes('intel')) {
            performance = 'medium';
        }
        ...
        const isWindows = navigator.platform.includes('Win');
        if (isWindows) {
            warnings.push('Windows platform detected - powerPreference ignored (expected behavior)');
        }
        ...
        return {
            ...
            adapter: false,
            performance: 'unknown',
            warnings: [`WebGPU health check failed: ${error?.message}`]
        };
    }
}

/**
 * Get comprehensive GPU status for debugging
 */
static async getGPUStatus(): Promise<any> {
    const health = await BrowserModelLoader.checkWebGPUHealth();
    const currentTier = testControl?.currentTier;
    ...
    return {
        ...
        engines: {
            loaded: Object.keys(testBedInfo.loadedModels || {}).length,
            current: this.currentEngine ? 'Available' : 'None'
        }
    };
}
	// ✅ NEW: Clean stale cache entries to prevent memory accumulation
// REPLACE the cleanupModelCache method:
static cleanupModelCache(): void {
    try {
        ...
        this.modelLoadingCache.forEach((entry, key) => {
            ...
            // ✅ ENHANCED: Multiple protection conditions
            const isWalkthroughActive = (window as any).walkthroughUI?.state?.isRunning;
            const isRecentlyUsed = (now - entry.timestamp) < 60000; // Used within last minute
            const isCurrentTier = entry.tier === testControl?.currentTier;
            
            // Protect entries that are:
            // 1. Active walkthrough entries
            // 2. Recently used entries
            // 3. Current tier entries
            // 4. Entries younger than TTL
            if ((isWalkthroughEntry && isWalkthroughActive) || 
                isRecentlyUsed || 
                isCurrentTier || 
                (now - entry.timestamp) < ttl) {
                
                protectedEntries.push(key);
                return; // Skip cleanup for protected entries
            }
            
            // Only cleanup truly stale entries
            if (now - entry.timestamp > ttl * 2) { // Double TTL for safety
                staleEntries.push(key);
            }
        });
        
        // Clean only genuinely stale entries
        staleEntries.forEach(key => this.modelLoadingCache.delete(key));
        
        if (staleEntries.length > 0) {
            console.log(`🧹 Cleaned ${staleEntries.length} stale cache entries (${protectedEntries.length} protected)`);
        }
        
        // Health report
        if (this.modelLoadingCache.size > 10) {
            console.warn(`⚠️ Cache growing large: ${this.modelLoadingCache.size} entries`);
        }
...
// ADD this method to BrowserModelLoader class:
private static updateCachedEngineProgress(tier: string, context?: ModelLoadingContext): void {
    try {
        const modelKey = context?.framework === 'Chapter7' && context.domain ? 
            `${tier}-${context.domain}` : tier;
        
        // Update progress for cached engines
        if (testBedInfo.loadedModels?.[modelKey]) {
            const entry = testBedInfo.loadedModels[modelKey];
            
            // Fix 0MB display issue
            if (entry.size === 'Loading...' || entry.size === '0MB') {
                entry.size = entry.estimatedSize || 'Cached';
            }
            
            // Fix 0% progress issue
            if (!entry.loadProgress || entry.loadProgress === 0) {
                entry.loadProgress = 100;
            }
            
            // Update status
            entry.status = '✅ Cached';
            entry.loadDuration = 0; // Instant load from cache
            
            BrowserLogger.log(`📦 Using cached ${tier} model: ${entry.name}`, 
                context?.framework === 'Chapter7' ? 'Chapter7' : 'T1-T10');
        }
        
        // Update live components
        BrowserModelLoader.updateLiveComponentsOnLoadComplete(tier, 'Cached Engine', context);
        
    } catch (error) {
        console.warn('Failed to update cached engine progress:', error);
    }
}
...
// ADD these methods to BrowserModelLoader class:
private static engineHealthCache = new Map<string, {
    healthy: boolean;
    lastCheck: number;
    consecutive_failures: number;
}>();

static trackEngineHealth(tier: string, healthy: boolean): void {
    const now = Date.now();
    const current = this.engineHealthCache.get(tier) || {
        healthy: true,
        lastCheck: 0,
        consecutive_failures: 0
    };
    
    this.engineHealthCache.set(tier, {
        healthy,
        lastCheck: now,
        consecutive_failures: healthy ? 0 : current.consecutive_failures + 1
    });
}

static isEngineHealthy(tier: string): boolean {
    const health = this.engineHealthCache.get(tier);
    if (!health) return true; // Assume healthy if not tracked
    
    const age = Date.now() - health.lastCheck;
    
    // Consider stale if not checked in 5 minutes
    if (age > 300000) return true;
    
    // Consider unhealthy if 3+ consecutive failures
    return health.healthy && health.consecutive_failures < 3;
}
...
private static async createEngineWithTimeout(model: string, options: any): Promise<any> {
    return new Promise(async (resolve, reject) => {
        ...
        try {
            BrowserLogger.log(`🔧 Creating MLCEngine for ${model} with optimized WebGPU`, 'ModelManager');
            
            // Enhanced options with WebGPU adapter
            const enhancedOptions = {
                ...options,
                // Add WebGPU-specific configurations if adapter is available
                ...(adapter && {
                    webgpu: {
                        adapter: adapter,
                        // Add tier-specific memory optimizations
                        maxBufferSize: testControl?.currentTier === 'Q8' ? 2048 * 1024 * 1024 : 1024 * 1024 * 1024
                    }
                }),
            ...
            // Create engine with enhanced options
            const engine = await webllm.CreateMLCEngine(model, enhancedOptions);
            ...
            // Add WebGPU status to engine
            (engine as any)._webgpuEnabled = !!adapter;
            (engine as any)._webgpuAdapter = adapter;
            
            BrowserLogger.log(`✅ MLCEngine created successfully for ${model} (WebGPU: ${adapter ? 'Enabled' : 'Disabled'})`, 'ModelManager');
            resolve(engine);
...
// ENHANCED: Atomic cache validation with locks
// REPLACE the cache validation logic in loadModel method:
...
try {
    ...
    if (cachedEntry && (Date.now() - cachedEntry.timestamp) < cacheTTL) {
        try {
            // SIMPLIFIED: Get cached engine with timeout protection
            const engine = await Promise.race([
                cachedEntry.promise,
                new Promise((_, reject) => 
                    setTimeout(() => reject(new Error('Cache validation timeout')), 3000) // Reduced from 5000
                )
            ]);
            
            // RELAXED: Basic health checks only
            const isWalkthroughContext = context?.framework === 'Chapter7';
            
            // ✅ SIMPLIFIED: Essential checks only
            const basicHealthChecks = [
                engine && typeof engine === 'object',
                engine.chat?.completions?.create,
                !engine._disposed && !engine._destroyed
            ];
            
            const healthyCount = basicHealthChecks.filter(check => check === true).length;
            const minRequiredChecks = isWalkthroughContext ? 2 : 3; // More lenient
            
            if (healthyCount >= minRequiredChecks) {
                // ✅ OPTIONAL: Quick functional test (skip if recent or walkthrough)
                const skipFunctionalTest = isWalkthroughContext || 
                    (Date.now() - cachedEntry.timestamp) < 30000; // Skip if loaded within 30 seconds
                
                if (skipFunctionalTest) {
                    console.log(`✅ Using cached engine for ${tier} (basic validation passed)`);
                    BrowserModelLoader.releaseCacheLock(cacheKey);
                    return engine;
                }
                
                // Quick functional test with short timeout
                try {
                    await Promise.race([
                        engine.chat.completions.create({
                        ...
                        new Promise((_, reject) => 
                            setTimeout(() => reject(new Error('Quick test timeout')), 2000) // Very short timeout
                        )
                    ]);
                    
                    console.log(`✅ Cached engine for ${tier} passed functional test`);
                    BrowserModelLoader.releaseCacheLock(cacheKey);
                    return engine;
                    
                } catch (testError) {
                    // Only invalidate cache for disposal errors
                    if (testError.message.includes('disposed') || testError.message.includes('NDArray')) {
                        console.log(`⚠️ Cached engine for ${tier} disposed - invalidating cache`);
                        BrowserModelLoader.modelLoadingCache.delete(cacheKey);
                    } else {
                        // Use cache anyway for other errors
                        console.log(`⚠️ Cached engine test failed but using anyway: ${testError.message}`);
                        BrowserModelLoader.releaseCacheLock(cacheKey);
                        return engine;
                    }
                }
            } else {
                console.log(`⚠️ Cached engine for ${tier} failed basic health (${healthyCount}/${basicHealthChecks.length}) - invalidating`);
                BrowserModelLoader.modelLoadingCache.delete(cacheKey);
            }
            
        } catch (cacheError) {
            console.log(`⚠️ Cache validation error for ${tier}: ${cacheError.message} - invalidating`);
            BrowserModelLoader.modelLoadingCache.delete(cacheKey);
        }
    }
} finally {
...
private static async performModelLoad(tier: string, context: ModelLoadingContext | undefined, startTime: number): Promise<any> {
	...
	// ✅ EARLY CACHE CHECK: Before doing any work
const cacheKey = context?.framework === 'Chapter7' && context.domain ? 
    `${tier}-${context.domain}-${context.framework}` : 
    `${tier}-${context?.framework || 'T1-T10'}`;

const existingCache = BrowserModelLoader.modelLoadingCache.get(cacheKey);
if (existingCache && BrowserModelLoader.isEngineHealthy(tier)) {
    try {
        const cachedEngine = await existingCache.promise;
        if (cachedEngine && BrowserModelLoader.validateEngineHealth(cachedEngine)) {
            // Update progress tracking for cached engine
            BrowserModelLoader.updateCachedEngineProgress(tier, context);
            BrowserModelLoader.trackEngineHealth(tier, true);
            
            console.log(`⚡ Fast cache hit for ${tier} - skipping full load`);
            return cachedEngine;
        }
    } catch (error) {
        console.log(`⚠️ Cache promise failed for ${tier}: ${error.message}`);
        BrowserModelLoader.trackEngineHealth(tier, false);
    }
}
	const frameworkLabel = context?.framework || 'T1-T10';
    const domainLabel = context?.domain ? ` for ${context.domain}` : '';
...
static initializeNDArrayMonitoring(): void {
    if (typeof window !== 'undefined' && (window as any).NDArrayMonitor) {
        // Create enhanced version without reassigning import
        this.enhancedCreateEngine = async (...args: any[]) => {
            const engine = await webllm.CreateMLCEngine(...args);
            if (engine) {
...
// Update the existing validateSystemCapabilities method
static async validateSystemCapabilities(): Promise<{ 
    canRun: boolean; 
    warnings: string[]; 
    recommendations: string[];
    ...
    try {
        // Check WebGPU with enhanced validation
        const webgpuHealth = await BrowserModelLoader.checkWebGPUHealth();
        
        if (!webgpuHealth.available) {
            warnings.push('WebGPU not supported');
            recommendations.push('Use Chrome/Edge 113+ with WebGPU enabled');
            canRun = false;
            chapter7Ready = false;
        } else if (!webgpuHealth.adapter) {
            warnings.push('No GPU adapter available');
            recommendations.push('Check GPU drivers and restart browser');
            recommendations.push('Ensure hardware acceleration is enabled in browser settings');
        } else {
            // Add performance-based recommendations
            if (webgpuHealth.performance === 'low') {
                recommendations.push('Consider upgrading GPU for better Q8 tier performance');
            }
            
            // Add Windows-specific guidance
            if (webgpuHealth.warnings.some(w => w.includes('Windows'))) {
                recommendations.push('Windows WebGPU detected - powerPreference warnings are expected and safe to ignore');
            }
            
            // Performance tier recommendations
            if (webgpuHealth.performance === 'high') {
                recommendations.push('🚀 High-performance GPU detected - optimal for Q8 tier models');
            } else if (webgpuHealth.performance === 'medium') {
                recommendations.push('💻 Integrated GPU detected - suitable for Q1/Q4 tiers, Q8 may be slower');
            }
        }
        ...
        if (typeof performance !== 'undefined' && performance.memory) {
            ...
            if (usedMB > limitMB * 0.8) {
                warnings.push('High current memory usage detected');
                recommendations.push('Consider refreshing the page to free up memory');
            }
            
            // Memory-based tier recommendations
            if (limitMB < 1024) {
                recommendations.push('💾 Limited memory detected - Q1 tier recommended for optimal performance');
            } else if (limitMB < 2048) {
                recommendations.push('💾 Moderate memory available - Q1/Q4 tiers recommended');
            } else {
                recommendations.push('💾 Sufficient memory for all tiers including Q8');
            }
        }
        ...
        // If too many T1-T10 issues, mark as not ready
        if (t1t10Issues >= tiers.length * 0.5) {
            canRun = false;
        }
        
        // Check Chapter 7 domain model availability
        const domains = ['appointment-booking', 'spatial-navigation', 'failure-diagnostics'];
        let domainIssues = 0;
        let domainOptimizationIssues = 0;
        ...
        for (const domain of domains) {
            for (const tier of tiers) {
                ...
                if (!domainValidation.isAvailable) {
                    ...
                    domainOptimizationIssues++;
                }
            }
        }
        
        // Domain-specific recommendations
        if (domainOptimizationIssues > 0) {
            const affectedDomains = domains.filter(domain => {
                return tiers.some(tier => {
                    const val = BrowserModelLoader.validateDomainModelAvailability(domain, tier);
                    return val.isAvailable && !val.domainOptimized;
                });
            });
            
            if (affectedDomains.length > 0) {
                recommendations.push(`📊 Domains using fallback models: ${affectedDomains.join(', ')} - performance may be suboptimal`);
            }
        }
        
        // Chapter 7 readiness assessment
        const totalDomainSlots = domains.length * tiers.length;
        if (domainIssues > 0) {
            if (domainIssues >= totalDomainSlots * 0.5) {
                chapter7Ready = false;
                recommendations.push('⚠️ Chapter 7 domain walkthroughs may have limited model availability');
            } else {
                recommendations.push('⚡ Chapter 7 ready with some fallback models');
            }
        }
        
        // Unified framework readiness
        unifiedReady = canRun && chapter7Ready && webgpuHealth.available;
        if (!unifiedReady) {
            if (!canRun) {
                recommendations.push('❌ Fix T1-T10 issues before using unified framework');
            }
            if (!chapter7Ready) {
                recommendations.push('❌ Fix Chapter 7 issues before using unified framework');
            }
            if (!webgpuHealth.available) {
                recommendations.push('❌ WebGPU required for unified framework');
            }
        }
        
        // Framework-specific success recommendations
        if (chapter7Ready && canRun && webgpuHealth.available) {
        ...
        if (userAgent.includes('chrome')) {
            if (webgpuHealth.available) {
                recommendations.push('✅ Chrome with WebGPU - optimal browser configuration');
            } else {
                recommendations.push('⚠️ Enable WebGPU in Chrome flags for best performance');
            }
        } else if (userAgent.includes('edge')) {
            recommendations.push('✅ Edge browser detected - good WebGPU support');
        } else if (userAgent.includes('firefox')) {
            recommendations.push('⚠️ Firefox detected - WebGPU support may be limited');
        } else {
            recommendations.push('❓ Unsupported browser - use Chrome or Edge for best results');
        }
        
        // Platform-specific recommendations
        const platform = navigator.platform.toLowerCase();
        if (platform.includes('win')) {
            recommendations.push('🖥️ Windows detected - ensure GPU drivers are up to date');
        } else if (platform.includes('mac')) {
            recommendations.push('🍎 macOS detected - WebGPU support varies by hardware generation');
        } else if (platform.includes('linux')) {
            recommendations.push('🐧 Linux detected - ensure proper GPU driver installation');
        }
        ...
        const errorMessage = `System validation error: ${error?.message}`;
...
// Update the existing initialize method
static initialize(): void {
    try {
        ...
        // Initialize WebGPU adapter early
        BrowserModelLoader.initializeWebGPUAdapter().then(adapter => {
            if (adapter) {
                BrowserLogger.log('✅ WebGPU adapter initialized successfully', 'ModelManager');
            } else {
                BrowserLogger.log('⚠️ WebGPU adapter initialization failed - using CPU fallback', 'ModelManager');
            }
        }).catch(error => {
            BrowserLogger.log(`❌ WebGPU adapter initialization error: ${error?.message}`, 'ModelManager');
        });
        ...
        if (typeof window !== 'undefined') {
            ...
            // Add WebGPU status to global debug
            (window as any).checkGPUStatus = () => BrowserModelLoader.getGPUStatus();
        }
        
        console.log('✅ ModelManager initialized with optimized WebGPU handling');
