...
private fallbackTrialCounting(data: any): { rate: number; successful: number; total: number; confidence: 'low'; method: string } {
   ...
   */
/**
 * ✅ FIXED: Enhanced token calculation with context awareness
 */
public calculateAverageTokens(data: any, context: 'average' | 'total' = 'average'): { 
  tokens: number; 
  samples: number; 
  method: string;
  totalTokens?: number;
  ...
  try {
      ...
      if (scenario.variants && Array.isArray(scenario.variants)) {
        ...
        for (const variant of scenario.variants) {
          ...
          if (variant.measuredProfile?.avgTokens) {
            ...
            totalTokens += variant.measuredProfile.avgTokens;
            method = 'measuredProfile';
          }
        }
      } else {
    ...
    // ✅ CONTEXT-AWARE: Return based on what's needed
    if (context === 'total') {
      console.log(`🔢 TOTAL TOKENS: ${totalTokens} from ${tokenCounts.length} trials (${method})`);
      return { 
        tokens: totalTokens, 
        samples: tokenCounts.length, 
        method,
        totalTokens 
      };
    } else {
      ...
      console.log(`🎯 AVG TOKENS: ${avgTokens.toFixed(1)} from ${tokenCounts.length} trials (total: ${totalTokens})`);
      return { 
        tokens: avgTokens, 
        samples: tokenCounts.length, 
        method,
...
private extractSummaryMetricsFromDetailedResults(result: any, approach: string): {
  ...
  avgLatency: number;
  avgTokens: number;      // Keep existing for compatibility
  totalTokens: number;    // ✅ NEW: Add total tokens
  mcdAlignment: number;
  ...
  try {
    ...
    // ✅ FIXED: Get both average and total tokens
    const avgTokenData = calculator.calculateAverageTokens(result, 'average');
    const totalTokenData = calculator.calculateAverageTokens(result, 'total');
    ...
    return {
      ...
      avgLatency: latencyData.latency,
      avgTokens: avgTokenData.tokens,
      totalTokens: totalTokenData.totalTokens || 0,
...
/**
 * Calculate proper MCD alignment based on actual trial compliance scores
 */
private calculateProperMCDAlignment(data: any, approach: string, result: any): number {
  try {
    // Extract trials from multiple possible data sources
    let trials: any[] = [];
    
    if (data.trials && Array.isArray(data.trials)) {
      trials = data.trials;
    } else if (result?.scenarioResults) {
      for (const scenario of result.scenarioResults) {
        if (scenario.variants && Array.isArray(scenario.variants)) {
          for (const variant of scenario.variants) {
            if (variant.trials && Array.isArray(variant.trials)) {
              trials.push(...variant.trials);
            }
          }
        }
      }
    ...
    if (trials.length === 0) {
      console.warn(`⚠️ No trials found for MCD calculation: ${approach}`);
      return this.getDefaultMCDAlignmentForApproach(approach);
    }

    const mcdScores = trials.map(trial => this.evaluateTrialMCDCompliance(trial, approach));
    const avgScore = mcdScores.reduce((a, b) => a + b, 0) / mcdScores.length;
    ...
    console.error(`❌ Error calculating MCD alignment for ${approach}:`, error);
    return this.getDefaultMCDAlignmentForApproach(approach);
  }
}
