...
class MetricCalculator {
  ...
  public calculateSuccessRate(data: any): { 
    ...
    try {
      ...
      // Strategy 1: Check scenario results with variants (NEW FORMAT)
            ...
            for (const variant of scenario.variants) {
              if (variant.trials && Array.isArray(variant.trials)) {
                for (const trial of variant.trials) {
                  totalTrials++;
                  if (this.isTrialSuccessful(trial)) {
                    successfulTrials++;
                  }
                }
              }
      ...
      // Strategy 2: Check domain metrics fallback
      if (totalTrials === 0 && data.domainMetrics) {
        method = 'domainMetrics';
        totalTrials = 1;
        successfulTrials = data.domainMetrics.overallSuccess ? 1 : 0;
      }
      
      const rate = totalTrials > 0 ? (successfulTrials / totalTrials) * 100 : 0;
      const confidence = totalTrials >= 5 ? 'high' : totalTrials >= 2 ? 'medium' : 'low';
      
      console.log(`📊 SUCCESS CALC: ${successfulTrials}/${totalTrials} = ${rate}% (${method}, ${confidence})`);
...
private handleResultAddedSafely(detail: ResultDetail): void {
  ...
  const approach = detail.result.approach || detail.result.approachType || 'default';
...
/**
 * ✅ REAL PROMPT BUILDING - Replace template substitution
 */
private buildPromptFromVariant(variant: any, trial: any): any {
  const promptTemplate = variant.promptTemplate || variant.template;
  
  // Check if this is a system role prompt
  const systemPromptMatch = promptTemplate.match(/^System:\s*(.*?)(?=\n\n|\nUser:|\n[A-Z][a-z]+:|$)/s);
  ...
  if (systemPromptMatch) {
    // System role approach: Extract system prompt
    systemPrompt = systemPromptMatch[1].trim();
    userPrompt = trial.userInput;
    fullPrompt = `${systemPrompt}\n\nUser: ${trial.userInput}`;
  } else {
    // MCD/Few-Shot approach: Keep template intact + append user input
    userPrompt = trial.userInput;
...
/**
 * ✅ REAL SUCCESS EVALUATION - Replace simple pass/fail
 */
private evaluateTrialWithTiers(output: string, trial: any): any {
  const domain = trial.domain || 'unknown';
  const tier = trial.tier || 'Q4';
  
  // Get default criteria when missing
  const defaultCriteria = this.getDefaultSuccessCriteria(domain, tier);
  ...
  const accuracy = maxScore > 0 ? score / maxScore : (failures.length === 0 ? 1.0 : 0.0);
  const success = accuracy >= minAccuracy && failures.length === 0;
  
  // Determine tier achievement
  let achievedTier = 'Q1';
  if (accuracy >= 0.9 && tokenCount <= adjustedBudget * 0.8) achievedTier = 'Q8';
  else if (accuracy >= 0.8 && tokenCount <= adjustedBudget * 0.9) achievedTier = 'Q4';
  
  return {
    success,
    accuracy,
    tokenCount,
    tokenEfficiency: tokenCount > 0 ? Math.min(1.0, adjustedBudget / tokenCount) : 1.0,
    tier: achievedTier,
    failures,
    score,
    maxScore,
...
private categorizeVariantApproach(variant: any): string {
  ...
  const templateLower = (variant.promptTemplate || '').toLowerCase();
  
  if (nameLower.includes('few-shot') || nameLower.includes('pattern') || templateLower.includes('example')) {
    return 'fewShot';
  }
  if (nameLower.includes('system') || nameLower.includes('role') || templateLower.startsWith('system:')) {
    return 'systemRole';
  }
...
private getDefaultSuccessCriteria(domain: string, tier: string): any {
  const baseCriteria = {
    'Appointment Booking': {
      'Q1': { minAccuracy: 0.6, maxTokenBudget: 45, maxLatencyMs: 400 },
      'Q4': { minAccuracy: 0.8, maxTokenBudget: 80, maxLatencyMs: 800 },
      'Q8': { minAccuracy: 0.9, maxTokenBudget: 150, maxLatencyMs: 1500 }
    },
    'Spatial Navigation': {
      'Q1': { minAccuracy: 0.7, maxTokenBudget: 40, maxLatencyMs: 350 },
      'Q4': { minAccuracy: 0.85, maxTokenBudget: 75, maxLatencyMs: 750 },
      'Q8': { minAccuracy: 0.95, maxTokenBudget: 120, maxLatencyMs: 1200 }
    },
    'Failure Diagnostics': {
      'Q1': { minAccuracy: 0.5, maxTokenBudget: 50, maxLatencyMs: 500 },
      'Q4': { minAccuracy: 0.75, maxTokenBudget: 100, maxLatencyMs: 1000 },
      'Q8': { minAccuracy: 0.9, maxTokenBudget: 200, maxLatencyMs: 2000 }
    }
  };
  
  return baseCriteria[domain]?.[tier] || baseCriteria['Appointment Booking']['Q4'];
}
...
private extractSummaryMetricsFromDetailedResults(result: any, approach: string): {
  ...
  const calculator = MetricCalculator.getInstance();
  const successData = calculator.calculateSuccessRate({ scenarioResults: trials });
  const latencyData = calculator.calculateAverageLatency({ scenarioResults: trials });
  const tokenData = calculator.calculateAverageTokens({ scenarioResults: trials });
...
private safeMemoryManager = {
  ...
  shouldCleanup(): boolean {
    ...
    const executionChecks = [
      ...
      { name: 'isUpdating', value: this.isUpdating },
      { name: 'processingQueue', value: this.processingQueue }
    ];
  ...
  performSafeCleanup(display: DomainResultsDisplay): Promise<void> {
    return new Promise((resolve) => {
      ...
      try {
        console.log('🧹 SAFE: Starting memory cleanup...');
  ...
  cleanOldTemplateCache(): void {
    ...
    const oldKeys = Array.from(cache.keys()).filter(key => {
      ...
      const timestamp = parseInt(keyParts[keyParts.length - 1]) * 1000; // Convert back to timestamp
  ...
  getLatestTimestamp(approaches: Map<string, any>): number {
    let latest = 0;
